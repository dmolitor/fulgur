...
Below we’ll demonstrate fitting an OLS model to predict airline arrival
delays using a large publicly available dataset of US airline on-time
performance.
...
fulgur fits its linear models via [Stochastic Gradient Descent
(SGD)](https://scikit-learn.org/stable/modules/sgd.html). We’ll compare
our SGD-based coefficients and Root Mean Squared Error (RMSE) on the
evaluation set to those from a standard OLS model.
